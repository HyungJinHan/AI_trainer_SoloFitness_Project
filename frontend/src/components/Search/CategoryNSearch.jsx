...
const Category = () => {
  ...
  function handleSearch(e) {
    if (
      SearchwordRef.current.value.length <= 1 ||
      SearchwordRef.current.value === undefined
    ) {
      Swal.fire("검색을 위해 두 글자 이상 입력해 주세요");
    } else {
      setMode(1);
  ...
  function categoryLower() {
    axios
      .post("http://localhost:8008/onecategory", { category: "하체" })
      .then((res) => {
  ...
  function categoryUpper() {
    axios
      .post("http://localhost:8008/onecategory", { category: "상체" })
      .then((res) => {
  ...
  function categoryEquip() {
    axios
      .post("http://localhost:8008/onecategory", { category: "기구운동" })
      .then((res) => {
  ...
  function categoryBarebody() {
    axios
      .post("http://localhost:8008/onecategory", { category: "맨몸운동" })
      .then((res) => {
  ...
  function categoryYoga() {
    axios
      .post("http://localhost:8008/onecategory", { category: "요가" })
      .then((res) => {
  ...
  function categoryStretch() {
    axios
      .post("http://localhost:8008/onecategory", { category: "스트레칭" })
      .then((res) => {
  ...
  function categoryFullbody() {
    axios
      .post("http://localhost:8008/onecategory", { category: "전신운동" })
      .then((res) => {
  ...
  function categoryPilates() {
    axios
      .post("http://localhost:8008/onecategory", { category: "필라테스" })
      .then((res) => {
  ...
  function categoryAerobic() {
    axios
      .post("http://localhost:8008/onecategory", { category: "유산소" })
      .then((res) => {
    ...
    return (
      <div className="CNS_search_main">
        {/* <input
          ...
          value="메인으로 돌아가기"
        /> */}
        <div className="CNS_search_info">검색</div>
              ...
              return (
                ...
                >
                  <div className="CNS_image">
                    <img
                      src={require(`../../static/images/JYY/ICON/mycollection/category/${item.thumb}`)}
                      alt="카테고리 이미지"
                    />
                  </div>
                  <p>{item.category}</p>
